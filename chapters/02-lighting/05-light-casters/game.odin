...
game_setup :: proc(game: ^Game) {
    ...
    // Initialize light
    game.light.pos         = { 0.0,  0.0, -5.0}
    game.light.dir         = { 0.0,  0.0, -1.0}
    game.light.cutOff      = math.cos_f32(glsl.radians_f32(12.5))
    game.light.cutOffOuter = math.cos_f32(glsl.radians_f32(17.5))
    game.light.diffuse     = { 0.5,  0.5,  0.5}
    game.light.specular    = { 1.0,  1.0,  1.0}
    game.light.constant    = 1.0
    game.light.linear      = 0.09
    game.light.quadratic   = 0.032
...
game_update :: proc(game: ^Game, dt: f64) {
    game.light.pos.x = math.sin_f32(f32(glfw.GetTime())) * 0.5;
    game.light.pos.y = math.cos_f32(f32(glfw.GetTime())) * 0.5;
}
...
game_render :: proc(game: ^Game) {
        ...
        shader_set_vec3(game.shader_program, "ambientLight", game.ambient_light)
        shader_set_vec3(game.shader_program, "lightSpot.pos", game.light.pos)
        shader_set_vec3(game.shader_program, "lightSpot.dir", game.light.dir)
        shader_set_vec3(game.shader_program, "lightSpot.diffuse", game.light.diffuse)
        shader_set_vec3(game.shader_program, "lightSpot.specular", game.light.specular)
        shader_set_float(game.shader_program, "lightSpot.cutOff", game.light.cutOff)
        shader_set_float(game.shader_program, "lightSpot.cutOffOuter", game.light.cutOffOuter)
        shader_set_float(game.shader_program, "lightSpot.constant", game.light.constant)
        shader_set_float(game.shader_program, "lightSpot.linear", game.light.linear)
        shader_set_float(game.shader_program, "lightSpot.quadratic", game.light.quadratic)
